...
class TestLovdataChunkerInitialization:
    ...
    def test_default_initialization(self):
        ...
        assert chunker.target == 768  # Updated from 512
        assert chunker.max == 8191
        assert chunker.min == 300  # New minimum token parameter
        assert chunker.overlap == int(768 * 0.15)
...
class TestStandardLawChunking:
    ...
    def test_chunk_standard_law(self, chunker, sample_standard_law_xml):
        ...
        # With min_tokens=300, small ledd chunks are merged
        # The two small ledd get merged into one chunk
        assert len(chunks) == 1, "Small ledd should be merged into 1 chunk"

        # Merged chunk contains both ledd
        ...
        assert "andre ledd" in chunks[0].text.lower()  # Both ledd in merged chunk
        assert chunks[0].metadata.get("merged") is True  # Marked as merged
...
class TestSimpleLawChunking:
    ...
    def test_chunk_simple_law(self, chunker, sample_simple_law_xml):
        ...
        With aggressive merging, small legalP elements are combined
        to meet minimum token requirements (default 300 tokens).
        """
        chunks = chunker.chunk(sample_simple_law_xml)

        # With aggressive merging, the two small legalP are combined into one chunk
        ...
        # If merged, verify the merged chunk contains both ledd
        if len(chunks) == 1:
            assert chunks[0].metadata.get("merged", False), "Should be marked as merged"
            assert "merged_count" in chunks[0].metadata
