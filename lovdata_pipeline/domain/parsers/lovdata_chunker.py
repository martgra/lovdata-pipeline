...
Optimized for 768-token chunks with hierarchical structure preservation.
Three-tier fallback strategy:
1. Standard laws: legalArticle → legalP (ledd)
2. Change laws: section → grouped legalP
3. Simple laws: main → legalP

Algorithm Flow Diagram: See docs/CHUNKING_ALGORITHM.md for detailed flow visualization.

Key Parameters:
- target_tokens: 768 (optimal for Norwegian legal text)
- max_tokens: 8191 (model context limit)
- min_tokens: 300 (merge threshold)
- overlap_ratio: 0.15 (15% overlap for context)
"""
...
from lovdata_pipeline.domain.models import Chunk
...
class LovdataChunker:
    ...
    def __init__(
        ...
        Args:
            target_tokens: Target number of tokens per chunk (768 optimal for Norwegian legal text)
            max_tokens: Maximum number of tokens per chunk
            min_tokens: Minimum tokens to avoid tiny chunks (reduces storage overhead)
    ...
    def _chunk_standard(self, root) -> list[Chunk]:
        ...
        for article in root.xpath('//article[@class="legalArticle"]'):
            ...
            for idx, ledd in enumerate(article.xpath('.//article[@class="legalP"]'), 1):
                ...
                # Skip chunks below minimum size (will be merged later)
                if tokens < self.min:
                    logger.debug(
                        f"Ledd {paragraph_ref}-{idx} below minimum ({tokens} < {self.min}), "
                        "will attempt to merge"
                    )
                    # Still process but flag for potential merging
                    pass
    ...
    def _split_large_ledd(
        ...
        has_lists = ledd_elem.find(".//ol") is not None or ledd_elem.find(".//ul") is not None
    ...
    def _merge_small_chunks(self, chunks: list[Chunk]) -> list[Chunk]:
        """Merge chunks below minimum size with adjacent chunks.

        Aggressive merging strategy for documents with very short articles:
        1. Accumulate consecutive chunks until >= min_tokens
        2. Keep merging as long as total <= max_tokens
        3. Ensures no chunk is smaller than min_tokens (unless impossible)
        ...
        for chunk in chunks:
            ...
            # Conditions: reached minimum OR would exceed max with next potential chunk
            should_flush = False

            # Have we reached minimum?
            if (
                buffer_tokens >= self.min
                or buffer_tokens + (buffer_tokens // len(buffer)) > self.max
            ):
                should_flush = True

            if should_flush:
                # Create merged chunk from buffer
                if len(buffer) == 1:
                    ...
                    logger.debug(
                        f"Merged {len(buffer)} small chunks "
                        f"({', '.join(buffer_ids)}) into {buffer_tokens} tokens"
                    )
        ...
        if buffer:
            if len(buffer) == 1:
                # Try to merge last small chunk with previous
                if buffer[0].token_count < self.min and merged:
                    prev_chunk = merged[-1]
                    combined_tokens = prev_chunk.token_count + buffer[0].token_count
                    if combined_tokens <= self.max:
                        ...
                        merged[-1] = Chunk(
                            ...
                            metadata={
                                **prev_chunk.metadata,
                                "merged_with": (
                                    prev_chunk.metadata.get("merged_with", [])
                                    + [buffer[0].chunk_id]
                                    if isinstance(prev_chunk.metadata.get("merged_with"), list)
                                    else [buffer[0].chunk_id]
                                ),
                                "merged": True,
                            },
                        )
                        logger.debug(
                            f"Merged last small chunk {buffer[0].chunk_id} "
                            f"({buffer[0].token_count} tokens) with previous"
                        )
                    else:
                        merged.append(buffer[0])
                else:
                ...
                # Multiple chunks in buffer - merge them
                merged_text = "\n\n".join(c.text for c in buffer)
                buffer_ids = [c.chunk_id for c in buffer]
                merged_chunk = Chunk(
                    chunk_id=buffer[0].chunk_id,
                    text=merged_text,
                    token_count=buffer_tokens,
                ...
                merged.append(merged_chunk)
                logger.debug(
                    f"Merged final {len(buffer)} small chunks "
                    f"({', '.join(buffer_ids)}) into {buffer_tokens} tokens"
                )
