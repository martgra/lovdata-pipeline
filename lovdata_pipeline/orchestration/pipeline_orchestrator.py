...
class PipelineOrchestrator:
    ...
    def run(
        ...
        to_process, removed = self._identify_files(
            lovlig, state, config.force, progress_tracker
        )

        # Stage 3: Process files
        processed, failed = self._process_files(
            to_process, state, progress_tracker
        )

        # Stage 4: Clean up removed files
        removed_count = self._cleanup_removed_files(
            removed, state, progress_tracker
        )
    ...
    def _identify_files(
        ...
        logger.debug(
            ...
            f"skipped {len(changed) - len(to_process)}"
        )
        ...
        if to_process:
            ...
            for idx, file_info in enumerate(to_process, 1):
                ...
                progress_tracker.update_file(
                    file_info.doc_id, idx - 1, len(to_process)
                )
