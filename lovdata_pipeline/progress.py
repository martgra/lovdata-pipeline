...
class RichProgressTracker:
    ...
    def __init__(self, console: Console | None = None):
        ...
        self.file_progress: Progress | None = None
        self.embedding_progress: Progress | None = None
    ...
    def _ensure_file_progress(self) -> Progress:
        """Ensure file progress instance exists and is started."""
    ...
    def _ensure_embedding_progress(self) -> Progress:
        """Ensure embedding progress instance exists and is started."""
        if self.embedding_progress is None:
        ...
        if not self._embedding_active:
            self.embedding_progress.start()
            self._embedding_active = True
        return self.embedding_progress
    ...
    def start_file_processing(self, total_files: int) -> None:
        """Start tracking file processing progress."""
        progress = self._ensure_file_progress()
        self.file_task_id = progress.add_task(f"Processing {total_files} files", total=total_files)

    def update_file(self, doc_id: str, current: int, total: int) -> None:
        """Update progress for current file being processed."""
        if self.file_progress and self.file_task_id is not None:
            # Update to show current file
    ...
    def end_file_processing(self) -> None:
        """End file processing progress tracking."""
        if self.file_progress and self.file_task_id is not None:
            self.file_progress.update(self.file_task_id, visible=False)
            self.file_task_id = None

        # Stop and clean up file progress instance
        if self.file_progress and self._file_active:
    ...
    def start_embedding(self, total_chunks: int) -> None:
        """Start tracking embedding progress within a file."""
        progress = self._ensure_embedding_progress()
        self.embedding_task_id = progress.add_task("[yellow]  └─ Embedding chunks", total=None)

    def update_embedding(self, chunks_embedded: int, total_chunks: int) -> None:
        """Update embedding progress."""
        if self.embedding_progress and self.embedding_task_id is not None:
    ...
    def end_embedding(self) -> None:
        """End embedding progress tracking."""
        if self.embedding_progress and self.embedding_task_id is not None:
            self.embedding_progress.remove_task(self.embedding_task_id)
            self.embedding_task_id = None

        # Stop and clean up embedding progress instance
        if self.embedding_progress and self._embedding_active:
